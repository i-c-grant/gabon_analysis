...
create_scatterplot <- function(data, title) {
  # Create scatterplot
  plot <- ggplot(data, aes(x = l4_agbd, y = avg_lvis_agb_pred)) +
    geom_point(size = .01) +
    labs(title = title,
         x = "L4A AGBD",
...
create_histogram <- function(data, title) {
  plot <- ggplot(data, aes(x = error)) +
    ...
    labs(title = paste("Histogram of", title),
         x = "Error (L4 AGBD - LVIS AGB Prediction)",
         y = "Frequency") +
    xlim(c(-500, 500)) +
    theme_classic() +
...
data_list <- map(files, \(x) {
  ...
  data <- data %>%
    mutate(year = year(time),
           ...
           error = l4_agbd - avg_lvis_agb_pred)
}
)
...
output_directory <- "/home/ian/projects/gabon/figures/lvis_vs_l4"
...
# Fit a linear model with l4_agbd and year as predictors
fit_model_year <- function(data) {
  data$year <- as.factor(data$year)
  model <- lm(avg_lvis_agb_pred ~ l4_agbd + year, data = data)
  summary(model)
}

# Fit a linear model with l4_agbd and beam_type as predictors
fit_model_beam_type <- function(data) {
  data$beam_type <- as.factor(data$beam_type)
  model <- lm(avg_lvis_agb_pred ~ l4_agbd + beam_type, data = data)
  summary(model)
}
